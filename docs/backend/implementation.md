...
- 프론트엔드, 데스크톱, 백엔드가 모두 JavaScript/TypeScript 생태계에 있어 타입 기반 개발이 쉽다.
- 백엔드 내부 공용 타입을 `apps/backend/src/shared.ts`에 모아 API 응답 구조 변경을 한 곳에서 확인할 수 있다.
- 이미지 목록 조회, 파일 ingest API처럼 I/O가 많은 서버 작업에 Node.js의 비동기 처리 모델이 잘 맞는다.

예를 들어 이미지 레코드는 `CatalogRecord`라는 타입으로 정의되어 있다. 이 타입에는 `imageId`, `bucket`, `fileName`, `metadata`, `syncStatus` 같은 필드가 들어간다.

관련 코드:

- [apps/backend/src/shared.ts](/Users/jinhyeokheo/Roylabs/Project/cuchen2/apps/backend/src/shared.ts:23)
- [apps/backend/src/shared.ts](/Users/jinhyeokheo/Roylabs/Project/cuchen2/apps/backend/src/shared.ts:40)
...
| `IngestModule` | API로 전달받은 파일 묶음의 MongoDB/MinIO 동기화 |
...
- 이미지 메타데이터는 제품번호, 촬영일시, 판정 결과, LOT, 공정 ID처럼 조건 검색이 많기 때문에 객체 저장소보다 DB 검색이 적합하다.
...
- `metadata`: 제품번호, 촬영일시, 공정코드, 판정 결과, LOT, 공정 ID, version 등
- `syncStatus`: 동기화 상태
...
현재 백엔드는 폴더를 계속 감시하지 않는다. 에이전트가 파일 생성을 끝낸 뒤 `POST /ingest/files`에 `path`와 `filebase`를 전달하면, 백엔드는 `{filebase}-{div}.{png|jpg|jpeg}`와 `{filebase}-{div}.json` 묶음만 처리한다.

선정 이유:

- 파일 생성 주체인 에이전트가 완료 시점을 알고 있으므로 backend watcher가 복사 완료 여부를 추정하지 않아도 된다.
- 백엔드는 요청받은 파일 묶음만 처리해 불필요한 폴더 순회를 줄인다.
- 4개 div 중 일부 파일이 없으면 `400 Bad Request`로 바로 드러낼 수 있다.

예를 들어 아래 요청은 `cuchen-test-top`, `cuchen-test-bot`, `cuchen-test-top-inf`, `cuchen-test-bot-inf`의 이미지/JSON 쌍을 처리한다.
...
- [apps/backend/src/ingest/api/ingest.controller.ts](/Users/jinhyeokheo/Roylabs/Project/cuchen2/apps/backend/src/ingest/api/ingest.controller.ts:12)
- [apps/backend/src/ingest/application/ingest.service.ts](/Users/jinhyeokheo/Roylabs/Project/cuchen2/apps/backend/src/ingest/application/ingest.service.ts:35)
...
### 2. API 기반 파일 묶음 ingest

백엔드는 시작할 때 입력 폴더를 자동 스캔하거나 계속 감시하지 않는다. 에이전트가 파일 생성 후 API를 호출하면 그 요청에 포함된 `path`와 `filebase`만 처리한다.

`POST /ingest/files`는 4개 div pair가 모두 있는지 먼저 확인한다. 하나라도 없으면 `400 Bad Request`로 거절하고 ingest를 시작하지 않는다. 저장에 성공한 입력 파일은 삭제하고, 실패한 pair는 `failed/` 폴더로 이동한다.

관련 코드:

- [apps/backend/src/ingest/api/ingest.controller.ts](/Users/jinhyeokheo/Roylabs/Project/cuchen2/apps/backend/src/ingest/api/ingest.controller.ts:12)
- [apps/backend/src/ingest/application/scan.service.ts](/Users/jinhyeokheo/Roylabs/Project/cuchen2/apps/backend/src/ingest/application/scan.service.ts:15)

### 3. 메타데이터 정규화

현장 JSON은 필드명이 항상 같지 않을 수 있다. 예를 들어 제품 ID가 `productId`, `product_id`, `productNo`, `productNumber`, `sku`, `제품번호`, `품번` 중 하나로 들어올 수 있다.
...
| `processId` | `processId`, `process_id`, `process`, `공정ID`, `cameraId`, `camera_id` |
| `version` | `version`, `metadataVersion`, `metadata_version` |

version이 입력 JSON에 없으면 기본값 `v1`을 `metadata.version`에 저장한다.

관련 코드:

- [apps/backend/src/shared.ts](/Users/jinhyeokheo/Roylabs/Project/cuchen2/apps/backend/src/shared.ts:78)
- [apps/backend/src/shared.ts](/Users/jinhyeokheo/Roylabs/Project/cuchen2/apps/backend/src/shared.ts:95)
...
`productNo`, `lotNo`, `processId`는 포함 검색이고, `query`는 이미지 ID, 파일명, 제품 ID, 검사 위치, 판정 결과, LOT, 공정 ID, version을 넓게 검색한다.
...
설계서의 `CatalogRecord` 주요 필드는 실제 백엔드 타입과 맞는다.
...
현재 metadata에는 `lotNo`, `processId`가 있고, `query` 통합 검색에도 들어간다. 또한 `GET /images/search?lotNo=...&processId=...`처럼 별도 query parameter로 직접 필터링할 수 있다.

프론트엔드 검색 필터의 LOT, 공정 ID 입력칸은 이 백엔드 query parameter로 전달된다.

구현 기준:

- `SearchFilters`에 `lotNo`, `processId`가 있다.
- `SearchRequestDto`에 `lotNo`, `processId`가 있다.
- MongoDB query에 `metadata.lotNo`, `metadata.processId` 조건이 있다.
